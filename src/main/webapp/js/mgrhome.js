...
function getTotalPending() {
	...
	xhttp.onreadystatechange = function() {
	if(xhttp.readyState == 4 && xhttp.status == 200){
		let total = JSON.parse(xhttp.responseText);
		console.log(total);
		document.getElementById("pendingHeader").innerText = `${total}`;
		}
	}
	
	xhttp.open("GET", "http://localhost:8080/ERS/gettotalpending.json");
...
function getPending() {
	...
	xhttp.open("GET", "http://localhost:8080/ERS/getpending.json");
	xhttp.onreadystatechange = function() {
	if(xhttp.readyState == 4 && xhttp.status == 200){
...
function getApproved() {
	...
	xhttp.open("GET", "http://localhost:8080/ERS/getapproved.json");
	xhttp.onreadystatechange = function() {
	if(xhttp.readyState == 4 && xhttp.status == 200){
...
function getDenied() {
	...
	xhttp.open("GET", "http://localhost:8080/ERS/getdenied.json");
	xhttp.onreadystatechange = function() {
	if(xhttp.readyState == 4 && xhttp.status == 200){
...
for(i=0; i < data.length; i++){
    ...
    cell4 = newRow.insertCell(4);
	cell4.innerHTML = data[i].reimbAuthor;
	cell5 = newRow.insertCell(5);
	
	var dateStr = JSON.parse(data[i].reimbSubmitted);
	
	var d = new Date(2021,05,03);
	cell5.innerHTML = d.getDate() + "-" + d.getMonth() + "-" + d.getFullYear();
	cell6 = newRow.insertCell(6);
    cell6.innerHTML = `<button class="pinkButton" onClick="onEdit(this)">Process</button>`;
